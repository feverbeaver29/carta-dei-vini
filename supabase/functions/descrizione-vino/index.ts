// supabase/functions/descrizione-vino/index.ts
...
serve(async (req) => {
  try {
    ...
    // 2) CARICA tutte le uve e denominazioni in memoria (sono poche, va benissimo)
    ...
    const grapesDetailed = grapeNames.map((gName) => {
      ...
      const matchPct = parsedGrapes.find((x) => norm(x.name) === key)?.pct ?? null;
    ...
    const pairingsChosen = pickDeterministic(pairingsPool, 3, wineSeed + "|pair");
    ...
    if (app) {
      ...
      if (acidBase != null) acidBase = Math.min(1, Math.max(0, acidBase + dA));
      if (tanninBase != null) tanninBase = Math.min(1, Math.max(0, tanninBase + dT));
      if (bodyBase != null) bodyBase = Math.min(1, Math.max(0, bodyBase + dB));
      if (sweetBase != null) sweetBase = Math.min(1, Math.max(0, sweetBase + dS));
      if (bubblesBase != null) bubblesBase = Math.min(1, Math.max(0, bubblesBase + dBu));
    }
    ...
    const defaultColor =
      app?.default_color ||
      (norm(wine.categoria).includes("rosso") ? "Rosso" :
       norm(wine.categoria).includes("bianco") ? "Bianco" :
       norm(wine.categoria).includes("rosa") || norm(wine.categoria).includes("rosato") ? "Rosato" :
       "ND");

    // 7) COSTRUISCI CONTEXT PER GPT (solo per la parte di testo, note/abbinamenti li abbiamo già)
    ...
    const fallbackPalate = clampChars(
      notesChosen.length
        ? `Al naso emergono ${notesChosen.join(", ")}; in bocca mantiene uno stile equilibrato e gastronomico.`
        : `Stile coerente con la denominazione, pensato per essere versatile a tavola.`,
      180
    );

    // 8) CHIAMATA GPT: genera hook + palate (2–3 frasi), non tocca note/abbinamenti
...
`.trim();
    ...
    const mini_card = {
      ...
      emojis: { notes: {}, pairings: {} } // puoi popolarle lato client se vuoi
    };
